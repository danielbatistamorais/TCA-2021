...
class Cache {
    ...
    generateHash(contents) {
        return rev_hash_1.default(contents);
    }
    /**
     * Makes cache path from a given file path and its contents
     */
    makeCachePath(filePath, contents, extname) {
        const relativeCachePath = utils_1.getCachePathForFile(this.appRoot, filePath);
        const hash = this.generateHash(contents);
        return path_1.join(this.cacheRoot, relativeCachePath, `${hash}${extname}`);
    }
    /**
    ...
    get(cachePath) {
        try {
            const contents = fs_extra_1.readFileSync(cachePath, 'utf8');
            utils_1.debug('reading from cache "%s"', cachePath);
            return contents;
        }
        catch (error) {
    ...
    set(cachePath, contents) {
        utils_1.debug('writing to cache "%s"', cachePath);
        fs_extra_1.outputFileSync(cachePath, contents);
    }
    /**
     * Clears all the generate cache for a given file
     */
    clearForFile(filePath) {
        utils_1.debug('clear cache for "%s"', filePath);
        const relativeCachePath = utils_1.getCachePathForFile(this.appRoot, filePath);
        fs_extra_1.removeSync(path_1.join(this.cacheRoot, relativeCachePath));
    }
    /**
     * Clears the cache root folder
     */
    clearAll() {
        fs_extra_1.removeSync(this.cacheRoot);
    }
}
exports.Cache = Cache;
/**
